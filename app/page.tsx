...
export default function Home() {
  ...
  return <AppShell title="Business dashboard" description="A live view of enquiries, quotes, jobs, installations, invoices and materials.">
    ...
    ].map(([step, title, text, href]) => <Link key={title} href={href} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#7ac400] text-sm font-black text-slate-950">{step}</span><h3 className="mt-4 font-bold text-slate-950">{title}</h3><p className="mt-1 text-sm text-slate-500">{text}</p></Link>)}</div></section>
